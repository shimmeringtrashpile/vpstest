...
public class ARGameLogic : MonoBehaviour
{
    ...
    public void LoadLocalReference()
    {
        ...
        {
            List<WayspotAnchorPayload> playloads = new List<WayspotAnchorPayload>();
            ...
            foreach (var wayspotAnchorPayload in storedData.Payloads)
            {
                ...
                playloads.Add(payload);
            }

            if (playloads.Count > 0)
            {
                var wayspotAnchors = wayspotAnchorService.RestoreWayspotAnchors(playloads.ToArray());
